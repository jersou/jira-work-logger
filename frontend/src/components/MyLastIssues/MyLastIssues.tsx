...
export const MyLastIssues: React.FC<MyLastIssuesProps> = ({data, config, issueClicked, setWorksLogged}) => {
  const [issues, setIssues] = useState([]);

  useEffect(() => {
    const timeout = setTimeout(() => getLastIssues(config).then(issues => setIssues(issues)), 1000)
  ...
  return <Accordion expanded={expanded === 'panel1'} elevation={3} onChange={handleChange('panel1')}>
    ...
    <AccordionDetails>
      <div style={{display: 'flex', flexDirection: 'row-reverse'}}>
        <Button variant="contained" onClick={async () => setIssues(await getLastIssues(config))}>
          <Refresh/>
        </Button>
      </div>
      <div className='last-issues'>
        {issues
          .filter((issue: IssueType) => !data.issues.map((i: IssueType) => i.key).includes(issue.key))
          .map((issue: IssueType) =>
            <div key={issue.key}>
              <Button variant="contained" color="primary" className="issue-btn" onClick={() => issueClicked({issue})}>
