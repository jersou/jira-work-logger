...
export interface WorkLogTableProps {
  ...
  onIssueKeyUpdated: (timeout: NodeJS.Timeout | undefined, setTimeoutRef: ((timeout: NodeJS.Timeout) => void), issue: IssueType, y: number, newData: { key?: string, comment?: string }) => void
}
  ...
  return (
    <TableContainer component={Paper} elevation={3}>
      <Table stickyHeader size="small" style={{overflow: 'hidden'}}>
        <WorkLogDateHeaderRow {...{data, removeColumn, setDateValue, addColumn}}/>
        <TableBody>
          <WorkLogTableIssueRows {...{data, removeRow, onIssueKeyUpdated, setHourValue, jiraUrl: config.jiraUrl}}/>
