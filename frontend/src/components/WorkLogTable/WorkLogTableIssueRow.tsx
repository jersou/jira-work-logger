...
function IssueCell(
  { issue, removeRow, y, onIssueKeyUpdated, jiraUrl }: IssueCellProps,
) {
  const [timeout, setTimeoutRef] = useState<NodeJS.Timeout>();
  return (
    <TableCell component="th" align="center" className="row-header">
      <Issue
        issue={issue}
        jiraUrl={jiraUrl}
        onDelete={() => removeRow({ num: y })}
        onKeyChange={(key) =>
          onIssueKeyUpdated(timeout, setTimeoutRef, issue, y, { key })}
        onCommentChange={(comment) =>
          onIssueKeyUpdated(timeout, setTimeoutRef, issue, y, { comment })}
      />
    </TableCell>
  );
}
...
export function totRow(data: WorkLogTableData, num: number): number {
  return round2(
    data.hours.map((col) => col[num]).reduce((tot, curr) => tot + curr, 0),
  );
}
...
export function WorkLogTableIssueRow({
  ...
  return (
    <TableRow className={y % 2 ? "odd" : "even"}>
      ...
      {data.dates.map((date, x) => (
        ...
        >
          <Hour
            hour={data.hours[x][y]}
            setHour={(hour) => setHourValue({ x, y, hour })}
          />
        </TableCell>
      ))}
      <TableCell
