...
const backendStatusTootilpTitleMap: { [key: string]: string } = {
  ...
  "ERROR":
    "The backend server status is impossible to determine (use the --wait-and-close parameter) or the server was DOWN even before opening the page",
      ...
      ? (
        <Tooltip title="Restart the server, and then refresh the page">
          <Button variant="contained" color="secondary">
            <ErrorOutline style={{ marginRight: 10 }} />The backend Server is
            down !
          </Button>
        </Tooltip>
      )
        ...
        <Tooltip title={backendStatusTootilpTitleMap[websocketState]}>
          <Button variant="contained" size="small" onClick={close}>
            <Stop /> Stop the Web App {websocketState === "OPEN"
              ? (
                <CheckCircleOutline
                  style={{ fontSize: 20, marginLeft: 10, color: "green" }}
                />
              )
              : ""}
          </Button>
