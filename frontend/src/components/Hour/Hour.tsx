...
export const Hour: React.FC<HourProps> = ({hour, setHour, ...props}) =>
  <div className="hour-cell">
    ...
    <div>
      <TextField className="hour" type="number" value={round2(hour)}
                 style={{backgroundColor: getHourColor(hour), borderRadius: 20, paddingLeft: 10}}
                 onWheel={e => {
                   e.deltaY > 0 ? setHour(decHouc(hour)) : setHour(incHour(hour));
                   e.preventDefault()
                 }}
                 onChange={(e) => setHour(Number(e.target.value))}
                 InputProps={{inputProps: {min: 0, max: 7.4, step}}} {...props}/>
    </div>
