...
export const importHamsterReportThunk =
...
export function importHamsterReportToState(
  ...
  dates.forEach(() =>
    state.data.hours.push(new Array(state.data.issues.length).fill(0))
  );

  hamsterReport.forEach(({ comment, hours, date }) => {
    const commentIndex = state.data.issues.map((issue) => issue.workLogComment)
      .indexOf(comment);
    if (commentIndex >= 0) {
      const dateIndex = state.data.dates.map((date) =>
        date.toISOString().substr(0, 10)
      ).indexOf(date);
      if (dateIndex >= 0) {
        state.data.hours[dateIndex][commentIndex] = round2(
          state.data.hours[dateIndex][commentIndex] + hours,
        );
      }
    }
  });
