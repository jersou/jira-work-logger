import {configureStore, createSlice, Draft, PayloadAction, ThunkDispatch} from "@reduxjs/toolkit";
...
function addRow(data: WorkLogTableData, issue: IssueType = {key: ""}): WorkLogTableData {
  return produce(data, (newData) => {
    newData.issues.push({...issue, reactKey: issue.reactKey || Math.random()});
...
const slice = createSlice({
  ...
  reducers: {
    ...
    issueClicked(state: Draft<AppState>, {payload: {issue}}: PayloadAction<{ issue: IssueType }>) {
      state.data = addRow(state.data, {key: issue.key, fields: {summary: issue.fields?.summary}});
    },
    ...
    websocketStatusChange(state: Draft<AppState>, {payload: {websocketState}}: PayloadAction<{ websocketState: WebsocketState }>) {
...
export const store = configureStore({reducer: slice.reducer, middleware: [thunk, stateToLocalStorageMiddleware]});
